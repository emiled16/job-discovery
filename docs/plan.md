## Job Discovery Platform: Vision, Target Architecture, and Ultra-Granular Task Plan
  ...
  | A04 | Add scheduler package skeleton | Create Beat entrypoint and schedule config loader. | Beat process starts with schedule registry. | Beat boot test confirms registered periodic task
  IDs. |
  ...
  | A23 | Seed command framework | Add deterministic seed runner command. | One-command local bootstrap. | Seed test verifies idempotent reruns (no duplicates). |
  ...
  | A41 | Admin manual sync trigger | Implement POST /api/v1/admin/companies/{id}/sync. | On-demand sync task enqueued. | Integration test asserts Celery task dispatch with run record. |
  ...
  | A49 | Run creation workflow | Create pipeline run record at task start. | Every sync has a traceable run ID. | Integration test verifies run record creation. |
  ...
  | A61 | Dashboard job list component | Implement card/table rendering and switcher. | Jobs readable in preferred format. | Component tests validate rendering parity card vs table. |
  ...
  | A64 | Job detail drawer/page | Implement full job view with metadata and actions. | Users can inspect details without losing context. | E2E test validates open/close/detail data integrity.
  |
  ...
  | A70 | Summary KPI components | Render totals, applied, application rate. | Users see progress metrics quickly. | Component tests validate numeric formatting and values. |
  ...
  | A74 | Admin manual sync action | Add per-company sync trigger controls and feedback. | Operator can trigger immediate ingestion. | E2E tests validate trigger creates visible new run. |
  ...
  | A76 | Structured logging baseline | Add JSON logging and correlation IDs across services. | Cross-service troubleshooting improved. | Log-format tests validate required fields on emitted
  logs. |
  ...
  | A78 | Security and input hardening | Add payload validation, safe URL handling, and output escaping baseline. | Reduced risk in admin/user inputs and fetch logic. | Security-focused tests
  validate rejected unsafe inputs. |
  | A79 | Full-stack compose E2E harness | Run end-to-end scenarios against live compose stack. | Product-level behavior guaranteed in runtime target. | E2E suite passes: browse/apply/view/
  summary/admin/sync flows. |
  | A80 | Engineering runbook and ops docs | Document setup, run, migration, seed, tests, troubleshooting, adapter extension. | Team can execute and extend system consistently. | Onboarding
  validation confirms documented steps produce working stack. |
  ...
  - Unit + integration + E2E suites pass on containerized stack.
  - Idempotency, deduplication, and job closure logic are proven by tests.

  ### Assumptions and Defaults

  - v1 runs with single seeded local user (local_user), no login UI.
